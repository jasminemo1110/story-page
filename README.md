...
> **标题字数提示**：屏幕上 16 个汉字以内单行展示，17 字以上会自动换行——若不喜欢自动换行的位置，用 `{br}` 手动指定断点。**打印时**标题字号会根据字数自动缩放（短标题保持满血，长标题平滑缩小，最多到 ~84px），确保始终单行呈现，所以无论你写多长都不需要为打印额外操心。
...
├── CLAUDE.md        # 维护说明：改这个 skill 时的注意事项（隐藏耦合、移植指引）
...
> 想 fork 或改造这个 skill？先读 [`CLAUDE.md`](CLAUDE.md)——里面记了几个不显眼、但改动时容易踩的坑（比如 schema 散在三处、移植到其他 agent 要改哪些地方）。
